...

{
    ...
    char liststr[MAXCHAR], **argkey, **argval, *str, *listname, *listbuf;
            ...
            if ((fp = fopen(listname, "r"))) {
                ...
                while (fgets(liststr, MAXCHAR, fp))
                    if (nim < MAXFILE) {
                        ...
                        error(EXIT_FAILURE,
                                "*Error*: Too many input catalogs in ",
                                liststr);
                fclose(fp);
            } else
                error(EXIT_FAILURE, "*Error*: Cannot open catalog list ",
                        listname);
        } else {
    ...
    NPRINTF(OUTPUT,
            ...
            prefs.time_diff, fields > 0.1 ? 1 : 2, fields,
            fields > 1.0 ? "s" : "", dets, dets > 1.0 ? "s" : "");
